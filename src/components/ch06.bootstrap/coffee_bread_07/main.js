...
function App() {
    ...
    const [products, setProducts] = useState([
        ...
        { id: 2, name: "크로와상", price: 2000, category: 'bread', stock: 222, image: 'croissant_08.png', description: "프랑스의 대표적인 베이커리 중 하나로, 층층이 쌓인 반죽에 버터를 추가하여 구워낸 과자입니다." },
    ...
    /* selectedId는 현재 선택이 된 항목의 상품 id 정보입니다. 프로그램 최초 시작시 1번이 선택되었다고 가정합니다. */
    const [selectedId, setSelectedId] = useState(1);

    /* 상품 목록에서 특정 상품 1개를 클릭했습니다. */
    const ClickArrived = (id) => {
        console.log(`샹품 아이디 : ${id}`);
    ...
    const InsertData = (formData) => {
        //formData 는 사용자가 입력한 폼 양식의 정보를 담고 있는 객체입니다.
        ...
        //newData 는 사용자가 입력한 데이터를 저장하고 있는 배열입니다.
        const newData = [{
            id: newId,
    ...
    /* 사용자가 상품 등록 화면에서 내용을 수정하고, [수정] 버튼을 눌렀습니다. */
    ...
    /* 사용자가 카테고리 추가 화면에서 내용을 수정하고, [추가] 버튼을 눌렀습니다. */
    const InsertCategory = (formData) => {
        //formData 는 신규 추가할 카테고리 입니다.
    ...
    const Ordering = (orderInformation) => {
        ...
        //a 와 b 는 각각 상품 1개를 의미하는 객체 정보(java의 bean으로 봐도 무방)
        products.sort((a, b) => {
            let aValue = a[column];
            let bValue = b[column];
    ...
    const CategoryChanged = (changedCategory) => {
        ...
        //필터링이 이루어지면, 정렬 함수를 다시 실행시켜 화면을 갱신시킵니다.
        Ordering(orderInfo);
    }
    ...
    return (
        <Card>
            ...
            <Card.Body>
                ...
                <Content
                    contents={filteredProducts} //보여줄 상품 목록
